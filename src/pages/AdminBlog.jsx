              ...
              <div className="mt-8 grid gap-8 lg:grid-cols-[1.1fr_1fr]">
                <form onSubmit={submitPost} className="rounded-xl border border-slate-200 bg-white p-5 shadow-soft">
                    ...
                    {editingId && currentImageUrl && !previewUrl && (
                      <label className="flex items-center gap-2 text-sm text-slate-700">
                        <input
                          ...
                          onChange={(event) => setForm((prev) => ({ ...prev, remove_featured_image: event.target.checked }))}
                        />
                  ...
                  <div className="mt-5 flex flex-wrap gap-2">
                    ...
                    {editingId && (
                      <button type="button" className="rounded-md border border-slate-300 px-4 py-2 text-sm font-semibold" onClick={resetEditor}>
                        Cancel Edit
                      </button>
                    )}
                  </div>
                    ...
                    {sortedPosts.map((post) => (
                      <article key={post.id} className="rounded-lg border border-slate-200 p-4">
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <Link to={`/blog/${post.slug}`} className="text-xs font-semibold text-brand-500 hover:text-brand-700" target="_blank" rel="noreferrer">
                            View
                          </Link>
                        </div>
